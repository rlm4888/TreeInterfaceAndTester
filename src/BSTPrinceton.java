 ...
 ******************************************************************************/
  ...
  * @author Kevin Wayne
  */
 public class BSTPrinceton<Key extends Comparable<Key>, Value> implements TInterface<Key, Value> {
    ...
    public void insertKV(BSTPrinceton<Integer, Integer> bst, int i, int j) {
    }

    public void deleteK(BSTPrinceton<Integer, Integer> bst, int i) {
    }
 }
 
 class TestBSTPrinceton implements Tester{
     ...
     public static void insertKV(BSTPrinceton<Integer, Integer> splayTree, Integer key, Integer value) {
         myAssert(!splayTree.contains(key));
         splayTree.put(key, value);
         myAssert(splayTree.contains(key));
     }
 
     public static void deleteK(BSTPrinceton<Integer, Integer> splayTree, Integer key) {
         myAssert(splayTree.contains(key));
     ...
     @Override
     public void test() {
        BSTPrinceton<Integer, Integer> bstPrinceton = new BSTPrinceton<>();
        doTests(bstPrinceton);
     }

     @Override
     public void insert() {
        BSTPrinceton<Integer, Integer> splayTree = new BSTPrinceton<>();
        insertKV(splayTree, null, null);
     }

     @Override
     public void remove() {
        BSTPrinceton<Integer, Integer> splayTree = new BSTPrinceton<>();
        myAssert(splayTree.contains(null));
         splayTree.delete(null);
         myAssert(!splayTree.contains(null));
     }

     public static void doTests(BSTPrinceton<Integer, Integer> bstPrinceton) {
         Integer[] keys = {0, 1, 2, 3, 4, 5, 6, 7};
         Integer[] values = {10, 11, 12, 13, 14, 15, 16, 17};
         int size = 0;
         for (int i = 0; i < keys.length; i++) {
             myAssert(bstPrinceton.size() == size++);
             insertKV(bstPrinceton, keys[i], values[i]);
             System.out.println(bstPrinceton);
         }
         for (int i = 0; i < keys.length; i++) {
             deleteK(bstPrinceton, keys[i]);
         }
     }
